...
import numpy as np
from library.model import GNN
from library.subgraph_model import KaryGNN, FinalLayer
from library.data import load_data, Batcher, Batch, Subgraph, stratified_ksplit
...
def add_arg(model_type, drop_ratio, num_layer, lr, batch_size):
    ...
    parser.add_argument(
        ...
        type=str,
        default=model_type,
        help="gin, gin-virtual, or gcn, or gcn-virtual, karygin, karygcn",
    )
    parser.add_argument(
        "--drop_ratio", 
        type=float, 
    ...
    parser.add_argument(
        ...
        type=int, 
        default=5, 
        help="size of subgraph (default: 5)"
    )    
    ...
    for task_name in tasks:
        ...
        for model_type in model_types:
            ...
            for r in rep_times:
                ...
                train_ds, val_ds_list, test_ds_list, id2graphlet_list = load_data()
                train_loader(self.train_ds, batch_size=self.conf.batch_size, shuffle=True,
                ...
                #train_dataset = PygPolymerDataset(name="prediction", root="data_pyg", task_name=task_name,repeat_times=r)
                #valid_dataset = PygPolymerDataset(name="prediction", root="data_pyg", task_name=task_name,repeat_times=r)
                ...
                elif args.gnn == "karygin":
                    model = KaryGNN(
                        gnn_type="gin",
                        num_layer=args.num_layer,
                        emb_dim=args.emb_dim,
                        drop_ratio=args.drop_ratio,
                        graphlet_sz=args.graphlet_size
                    ).to(device)
                    model = FinalLayer(
                        model,
                        emb_dim=args.emb_dim,
