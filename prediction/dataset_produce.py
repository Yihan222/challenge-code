...
class SmilesRepeat():
    def __init__(self, repeat_times, task_name='O2',root='data_pyg/prediction/') -> None:
        self.repeat_times = repeat_times
        self.task_name = task_name
        self.root = osp.join(root,task_name)
        self.raw_file = osp.join(root,task_name,task_name+'_raw_1',task_name+'_raw_120.csv')
    ...
    def repeat(self):
        ...
        if not os.path.exists(self.raw_file):
            ...
            new_root = '{}_raw_{}'.format(self.task_name,self.repeat_times)
            path1 = osp.join(self.root, new_root)
            if not os.path.exists(path1):
                os.makedirs(path1)
            newcsvpath = path1+'/{}_raw_120.csv'.format(self.task_name)
