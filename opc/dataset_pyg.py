...
class PygPolymerDataset(InMemoryDataset):
    def __init__(
        ...
        if _use_concat_train:
            self.root = osp.join(root, name, self.task_type, 'concat', str(self.repeat_times),self.set_name)
        else:
            raw_root = osp.join(root, name, self.task_type, str(self.repeat_times))
            if not osp.exists(raw_root):
                dataproduce = SmilesRepeat(self.repeat_times, self.task_type, root='./data_pyg/prediction/')
                dataproduce.repeat()
            self.root = osp.join(raw_root,self.set_name)
            if not osp.exists(self.root):
                gsplit(self.repeat_times,self.task_type)
